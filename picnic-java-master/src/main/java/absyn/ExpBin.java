...
import interpreter.ValueBool;
import interpreter.ValueInt;
...
public class ExpBin extends Exp {
   ...
   public Value eval(Table<Value> memory, List<Fun> functions){
      ...
      switch (op){
         ...
         case EQ: return new ValueBool(((ValueBool)x).value == ((ValueBool)y).value);
         case NE: return new ValueBool(((ValueBool)x).value != ((ValueBool)y).value);
         default: return new ValueInt(((ValueInt)x).value);
      }
   }
}
